...
async function exerciseStartupFailure(
  ...
  try {
    ...
    for (let i = 0; i < 30 && !stderr.includes(expectedError); i++) {
    ...
    expect(status.startup?.error).toContain(expectedError);
...
describe("MCP startup registration", () => {
  it("registers tools before a broken Firefox autolaunch can terminate startup", async () => {
    await exerciseStartupFailure({}, "FRX_FIREFOX_BIN is unset");
  }, 10_000);

  it("registers tools before FRX_ENV_ID resolution fails", async () => {
    await exerciseStartupFailure({ FRX_AUTOLAUNCH: "0", FRX_ENV_ID: "invalid/env/id" }, "invalid characters");
  }, 10_000);
});
