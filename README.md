...
在你的 **AI 编码工具**（Cursor / Claude Code / Codex 等）的对话框里直接说：

> 帮我安装并配置这个 MCP 工具：**frx-director-mcp**
> 项目地址：https://github.com/WhiteNightShadow/frx-director-mcp

AI 会自动完成 **克隆 → 装依赖 → `npm run build` → 把 MCP Server 写进你客户端的 MCP 配置** 的全部流程。装完后按上面「前置条件」把浏览器准备好，再用下面「🟢 一键贴给你的 AI」那段开跑。
...
然后把这个 server 加进你客户端的 MCP 配置（`<克隆路径>` 换成你的实际路径）：
...
{ "mcpServers": { "frx-director": {
  "command": "node",
  "args": ["<克隆路径>/frx-director-mcp/dist/index.js"]
} } }
```
</details>
...
```bash
claude mcp add frx-director -- node <克隆路径>/frx-director-mcp/dist/index.js
```
</details>

> 可选环境变量（都有默认值；走方式一让 AI 帮你填）：`FRX_MARIONETTE_PORT`(默认 2828)、`FRX_JSX_PROMPT_SRC`(指向 firefox-reverse 的 `AgentPanel.jsx`，让 director 提示词跟浏览器 UI 同步)、`FRX_WORKSPACE_ROOT`(工作目录根)、`FRX_AUTOLAUNCH=1` + `FRX_FIREFOX_BIN`/`FRX_PROFILE`(让 MCP 自动拉起带 marionette 的浏览器)。详见 `.env.example`。
