...
director (Claude / GPT)
   │  stdio (MCP)
   ▼
frx-director-mcp ──── 10 tools ──── BrowserBridge（Marionette 默认 / File 备用）
...
| `agent_tools` | 列出浏览器侧 worker 可用的逆向工具清单（名称 / 说明 / 是否需确认 / 参数名）——只读，供 director 了解 worker 能力、把 guidance 写到点上（不直接调用，推进仍走 worker）|
