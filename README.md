...
2. 以 Marionette + 系统权限启动浏览器（chrome 上下文执行特权 JS 所必需）。macOS 推荐走 `.app` 的 LaunchServices 启动链路：
   ...
   open -n -a "/Applications/Firefox Reverse.app" --args \
       ...
       -profile "<你的 profile>"
   ```
   Windows / Linux 可直接用 Firefox Reverse 可执行文件带同样参数启动。默认 Marionette 端口是 `2828`；自定义端口请配置 `FRX_MARIONETTE_PORT` 并使用自动拉起，启动器会通过官方 pref/环境变量传入。也可以配置 `FRX_AUTOLAUNCH=1` + `FRX_FIREFOX_BIN` + `FRX_PROFILE`，由本服务自动拉起；macOS 下 `FRX_FIREFOX_BIN` 可指向 `/Applications/Firefox Reverse.app` 或 `.app/Contents/MacOS/firefox`，MCP 会自动转换为 `open -n -a ... --args`。
3. Node.js ≥ 18。
...
- MCP initialize 握手从 `package.json` 读取实际版本，避免客户端看到旧版本号；7 个测试文件、25 项测试通过，实际 stdio 验证 23 个入口工具、68 个浏览器工具及环境列表。
- 配合 Firefox-Reverse v0.25.0 使用；更新后重启 MCP 客户端连接。发布包不包含个人 `.env` 或浏览器 profile。
...
- **浏览器工具目录补全**：`agent_tools` 改为读取浏览器实际 backend 注册表，修复旧静态 stub 只返回 38/66 个工具的问题；新增 `missingDeclared` 一致性诊断。
- **失败保持可诊断**：`FRX_FIREFOX_BIN`、`FRX_ENV_ID` 或浏览器启动配置错误不再直接退出 MCP；`frx_status` 返回 `mcpToolsRegistered` 与 `startup.phase/error`，用户修正浏览器配置后可明确复查。
- **进程级回归测试**：真实启动 MCP stdio 子进程，在故意破坏 Firefox autolaunch 的情况下验证 `initialize` / `tools/list` 仍成功，且包含 `frx_status`、`agent_tools`、`agent_call_tool`、不包含外部宿主的 `open_url`。
