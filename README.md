...
| 职责 | 由谁承担 | Token 开销 |
|---|---|---|
| **工具执行** —— signer_trace、批量 run_node 试算法、JSVMP 逐指令、page_eval、字节比对… | worker 模型（**推荐 DeepSeek-v4-pro**，相对 director 仍便宜；或 Qwen / GLM）| 高频、量大；在浏览器引擎内执行，使用你在浏览器中配置的 Key |
...
1. 安装 [firefox-reverse](https://github.com/WhiteNightShadow/firefox-reverse) 浏览器，并在其 Agent 设置中为 worker 模型配置 API Key（本服务不接触该 Key）。**默认推荐 DeepSeek-v4-pro**（推理强、更适合难站）；也支持 通义千问 / GLM 等。pro 在长工具循环中偶尔退化为纯文本，但 director 会通过 `driftHint` / `runlog` 发现，并以一条"先调工具"的具体指令把它拉回。
...
- ready=true → 进入下面流程。
- bridgeConnected=false 或 hasKey=false → 把返回的 note 用一句话告诉用户、引导其补齐（用 -marionette 启动浏览器，或在浏览器 Agent 设置里为 worker 模型（推荐 deepseek-v4-pro；或 qwen / glm）配置 Key）；待其完成后再次 frx_status 确认，再继续。
